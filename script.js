...
function displayQuestion() {
  ...
  if (currentQuestion === questions.length - 1) {
    // Special handling for the "Will you marry me?" question
    const buttonYes = document.createElement("button");
    buttonYes.textContent = "Yes";
    buttonYes.onclick = function () {
      checkAnswer(this);
    };
    optionsContainer.appendChild(buttonYes);
    ...
    optionsContainer.appendChild(buttonNo);
  } else {
  ...
  if (!attemptMade) {
    ...
    if (selectedIndex === questions[currentQuestion].correctAnswer) {
      ...
      if (currentQuestion !== questions.length - 1) {
        ...
        // Last question (marriage proposal)
        showMarriageGifts();
      }
    } else {
      // Incorrect answer
